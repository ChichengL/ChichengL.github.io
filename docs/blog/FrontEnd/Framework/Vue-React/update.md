...
- **Vue 的性能优势**Vue 的精细化依赖追踪在理论上性能更高，因为它从源头上避免了不必要的组件渲染和 Diff 计算。在组件更新频繁、数据依赖关系复杂的场景下，Vue 的自动优化能力通常表现更优（[from](https://bytedance.larkoffice.com/wiki/wikcnACHy3Ztisi0sRQXbaEGcCd)）。Vue 3 的编译时优化（`PatchFlag`）更是将这种优势发挥到极致，它可以跳过静态内容，只对动态绑定的部分进行 Diff（[from](https://blog.51cto.com/wds0/13660908)）。
